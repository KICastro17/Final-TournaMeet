...
{
    // Suppress errors for ALTER TABLE — it's fine if columns already exist
    mysqli_report(MYSQLI_REPORT_OFF);

    $conn->query("ALTER TABLE tournaments ADD COLUMN event_time TIME NULL");
    $conn->query("ALTER TABLE tournaments ADD COLUMN registration_deadline DATETIME NULL");
    $conn->query("ALTER TABLE tournaments ADD COLUMN is_closed TINYINT(1) NOT NULL DEFAULT 0");
    $conn->query("ALTER TABLE tournaments ADD COLUMN requirements TEXT NULL");
    $conn->query("ALTER TABLE tournaments ADD COLUMN organizer_note TEXT NULL");
    $conn->query("ALTER TABLE tournaments ADD COLUMN views_count INT NOT NULL DEFAULT 0");
    $conn->query("ALTER TABLE tournaments ADD COLUMN created_by VARCHAR(50) NULL");
    $conn->query("ALTER TABLE tournaments ADD COLUMN prize_pool DECIMAL(10,2) NOT NULL DEFAULT 0");
    $conn->query("ALTER TABLE tournaments ADD COLUMN registration_fee DECIMAL(10,2) NOT NULL DEFAULT 0");
    $conn->query("ALTER TABLE tournaments ADD COLUMN slots INT NOT NULL DEFAULT 0");
...
{
    if (empty($tournament['event_date'])) return false;
    return strtotime($tournament['event_date'] . ' 23:59:59') < time();
}

function deadlinePassed(array $tournament): bool
{
    if (empty($tournament['registration_deadline'])) return false;
...
function incrementTournamentViewOnce(mysqli $conn, int $tournamentId): void
{
    ...
    if (in_array($tournamentId, $_SESSION['tournament_views'], true)) return;
...
function hasDuplicateMemberNames(string $members): bool
{
    ...
    $lowered = array_map(static fn($v) => strtolower($v), $lines);
...
function sanitizeTournamentPayload(array $input): array
{
    ...
    $errors = [];
    if ($title === '' || strlen($title) > 150)                            $errors[] = 'Title is required and must be 150 characters or less.';
    if ($description === '')                                               $errors[] = 'Description is required.';
    if ($sport === '')                                                     $errors[] = 'Sport is required.';
    if ($date === '' || !preg_match('/^\d{4}-\d{2}-\d{2}$/', $date))     $errors[] = 'A valid event date is required.';
    if ($time !== '' && !preg_match('/^\d{2}:\d{2}$/', $time))           $errors[] = 'Invalid event time format.';
    if ($location === '' || strlen($location) > 255)                      $errors[] = 'Location is required and must be 255 characters or less.';
    if ($registrationFee < 0 || $prizePool < 0)                          $errors[] = 'Fees and prize pool cannot be negative.';
    if ($slots < 1)                                                       $errors[] = 'Slots must be at least 1.';
